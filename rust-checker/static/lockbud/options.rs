...
impl Options {
    ...
    pub fn parse_from_args(flags: &[String]) -> Result<Self, Box<dyn Error>> {
        ...
        let output_file = matches.value_of("out").map(|s| s.to_string());
...
mod tests {
    ...
    fn test_parse_from_str_blacklist_ok() {
        let options = Options::parse_from_str("-k deadlock -o test.json -b -l cc,tokio_util,indicatif").unwrap();
        ...
        assert!(
            options.output_file == Some("test.json".to_owned())
        );
    }
