...
fn main() {
    ...
    if std::env::var("ATOMVCHECKER_LOG").is_ok() {
        let e = env_logger::Env::new()
            .filter("ATOMVCHECKER_LOG")
            .write_style("ATOMVCHECKER_LOG_STYLE");
        env_logger::init_from_env(e);
    }
    // Get any options specified via the ATOMVCHECKER_FLAGS environment variable
    let options = Options::parse_from_str(&std::env::var("ATOMVCHECKER_FLAGS").unwrap_or_default())
        .unwrap_or_default();
    // debug!("ATOMVCHECKER options from environment: {:?}", options);
    ...
    let result = rustc_driver::catch_fatal_errors(|| {
            ...
            .any(|arg| arg.starts_with(&print))
        {
            ...
            // anything. We also don't want to the caller to know that ATOMVCHECKER adds configuration
